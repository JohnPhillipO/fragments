...
async function readFragmentData(ownerId, id) {
  logger.info({ oID: ownerId, ID: id }, `readFragmentData() from S3`);
  // Create the PUT API params from our details
  const params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    // Our key will be a mix of the ownerID and fragment id, written as a path
...
async function deleteFragment(ownerId, id) {
  ...
  // Create the Delete params, with the table name and the key
  const paramsDb = {
    TableName: process.env.AWS_DYNAMODB_TABLE_NAME,
    Key: { ownerId, id },
  };
  ...
  const command2 = new DeleteCommand(paramsDb);

  try {
    await ddbDocClient.send(command2);
    await s3Client.send(command);
  } catch (err) {
    logger.error({ err, params, paramsDb }, 'Error deleting fragment data from S3 + DynamoDB');
